...
$.fn.extend({
    animateCss: function (animationName) {
        ...
        this.addClass('delay animated ' + animationName).one(animationEnd, function() {
            $(this).removeClass('animated ' + animationName);
        });
    }
});

console.log('Hi There! It\'s always nice to check the console!');
...
function pp() {
    ...
    onLeave: function(index, nextIndex, direction){
        if(index==1) {
            $('.text').removeClass('delay-extended');
        }
    },
    afterLoad: function(anchorLink, index){
        if(index==3) {
            $('#iecse-vid').get(0).play();
        }
        ...
        if(index == 1) {
            ...
            $('#about-heading').animateCss('fadeIn');
            $('#about-line').animateCss('delay-extended fadeIn');
        }
            ...
            $(".img-box-container").children().eq(random).addClass('animated tada');
            setTimeout(BackToOriginal, 1200);

            function BackToOriginal() {
                $(".img-box-container").children().eq(random).removeClass('animated tada');
            }

        }
    },
    afterRender: function(){
        $('.text').addClass('delay-extended');
    },
});
}
...
jQuery(document).ready(function() {
    ...
    jQuery('#abt').on('click', function() {
        if ( $(window).width() > 739) {
            TweenMax.to(jQuery('.about-me'), 0.8, {x : -450, ease: 'Power4.easeInOut'});
            TweenMax.to(jQuery('#one'), 0.1, {x : -300, ease: 'Power0.easeInOut'});
        }
        else {
            TweenMax.to(jQuery('.about-me'), 0.8, {x : -250, ease: 'Power4.easeInOut'});
            TweenMax.to(jQuery('#one'), 0.1, {x : -80, ease: 'Power0.easeInOut'});
        }
        TweenMax.to(jQuery('#three h1'), 0.1, {x : -250, ease: 'Power4.easeInOut'});
        jQuery('#landing-logo').addClass('change-opacity');
        jQuery('#landing-heading').addClass('change-opacity');
        jQuery('.text').addClass('change-opacity');
        jQuery('.svg-wrapper').addClass('change-opacity');
        jQuery('#three h1').addClass('change-opacity');
        jQuery('.lower').addClass('change-opacity');
        jQuery('.upper').addClass('change-opacity');
        jQuery('#line').addClass('change-opacity');
        jQuery('#about-heading').addClass('change-opacity');
        jQuery('#about-line').addClass('change-opacity');
        jQuery('#about-desc').addClass('change-opacity');
        jQuery('#about-desc-two').addClass('change-opacity');
        jQuery('#line-four').addClass('change-opacity');
        TweenMax.to(jQuery('.fa-times'), 0.2, {rotation:0});
    });

    jQuery('.fa-times').on('click', function() {
        TweenMax.to(jQuery('#one'), 0.1, {x : 0, ease: 'Power0.easeInOut'});
        TweenMax.to(jQuery('.about-me'), 0.8, {x : 450, ease: 'Power4.easeInOut'});
        TweenMax.to(jQuery('#three h1'), 0.1, {x : 0, ease: 'Power4.easeInOut'});
        jQuery('#landing-logo').removeClass('change-opacity');
        jQuery('#landing-heading').removeClass('change-opacity');
        jQuery('.text').removeClass('change-opacity');
        jQuery('.svg-wrapper').removeClass('change-opacity');
        jQuery('#three h1').removeClass('change-opacity');
        jQuery('.lower').removeClass('change-opacity');
        jQuery('.upper').removeClass('change-opacity');
        jQuery('#line').removeClass('change-opacity');
        jQuery('#about-heading').removeClass('change-opacity');
        jQuery('#about-line').removeClass('change-opacity');
        jQuery('#about-desc').removeClass('change-opacity');
        jQuery('#about-desc-two').removeClass('change-opacity');
        jQuery('#line-four').removeClass('change-opacity');
        TweenMax.to(jQuery(this), 1, {rotation:0});
    });
    TweenMax.set(jQuery('#contactHeading'), {autoAplha: 0});
